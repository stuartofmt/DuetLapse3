...
def init():
    ...
    global keeplogs, novideo, deletepics, maxffmpeg
    #Derived  globals
    ...
    if (phase == 'startup'):
        if win:
            ...
            if (not keeplogs and len(pidlist) == 1):  #only delete logs if no other processes running
                subprocess.call('del /q "'+topdir+'\\"*.log'+debug, shell=True)   #Note position of last " so that shell expands *.log portion 
        else:
            for dirs in dirlist:
                split_dirs = dirs.split("-", 1)
                dirpid = split_dirs[0]
...
def oneInterval(cameraname, camera, weburl, camparam):
    ...
    if ((seconds > 0 and seconds < elap) and (dontwait or zn >= 1)):
        checkForPause(zn)
        logger.info(cameraname+': capturing frame '+str(frame)+' at layer '+layer+' after '+str(seconds)+' seconds')
        onePhoto(cameraname, camera, weburl, camparam)
...
def makeVideo():
    ...
    if (camera2 != ''): postProcess('Camera2', camera2, weburl2, camparam2, vidparam2)
...
class MyHandler(SimpleHTTPRequestHandler):
    ...
    def update_content(self):
        ...
        info =     ('<h3><pre>'
                    'This instance is using the following options:<br>'
                    +thisrunninginstance+'<br><br>'
                    'Logs and Videos are located here:<br>'
                    +topdir+'<br><br>'
                    'Images are located here:<br>'
                    +imagelocation+'<br><br>'
                    'The logfile for this instance is:<br>'
                    +logname+
                    '</pre></h3>'   
                    )    
        
        status =    ('DuetLapse3 Version '+duetLapse3Version+'<br>'
                    '<h3>'
                    'Status of printer on '+duet+'<br>'
        ...
        buttons =   ('<div class="divider">'
                    ...
                    '<input type="submit" value="Info" style="background-color:green"/>'                    
                    '</form>'
                    '</div>'
                    '<div class="divider">'                                     
                    '<form action="http://'+referer+'">'
    ...
    def do_GET(self):       
        ...
        if ((query_components and not query_components.get('command') and not query_components.get('delete')) or (not query_components and self.path != '/')):
        ...
        if(query_components.get('delete')): 
            ...
            try:
                subprocess.call(cmd, shell = True)
                logger.info('!!!!! http delete processed for '+file+' !!!!!')
            except:
                logger.info('!!!!! An error occured trying to delete '+file+' !!!!!')
    ...
    def display_dir(self,path):
        ...
        if trailing_slash:   # this is a dir request
            ...
            requested_dir = requested_dir.replace('%CB%B8',u'\u02f8') #undoes raised colons that were replaced by encoding
            try:
    ...
    def list_dir(self, path):  #Copied from super class
        try:
            ...
            logger.info('No permission or directory not found')
            return None
        ...
        for name in list:
            if not name.startswith(pid) and not name.endswith('.jpeg'): continue  #only display for this instance
            ...
            r.append('<tr><td><a href="%s">%s</a></td>\n'
                     ...
                     html.escape(displayname, quote=False)))
            if not child_dir and ('Camera' in displayname):
    ...
    while(capturing):  #action can be changed by httpListener or SIGINT or CTL+C
        ...
        if(duetStatus == 'disconnected'): #provide some resiliency for temporary disconnects
            ...
            if (disconnected > 10): #keep trying for a while just in case it was a transient issue
                ...
                nextactionthread = threading.Thread(target=nextAction, args=('terminate',)).start()   #Note comma in args is needed
...
def nextAction(nextaction):  #can be run as a thread
    ...
    if (capturing):
        action = 'start'
        logger.info('++++++ Entering '+action+' state ++++++')
        capturethread = threading.Thread(target=captureLoop, args=()).start()
        #capturethread.start()
